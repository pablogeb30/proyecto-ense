...
import DefaultPoster from '../movie/images/poster.jpg'
...
export default function Movie() {
	...
	const { id } = useParams()
	const { movie, update, status } = useMovie(id)
	...
	return <Shell>
		...
		<div className = 'mx-auto w-full max-w-screen-2xl p-8'>
			<Header movie = { movie } />
			<Info movie = { movie } update = { update } status={ status }/>
			<View movie = { movie } />
			<Cast movie = { movie } />
...
function Info({ movie, update, status }) {

	const [updatedOverview, setUpdatedOverview] = useState(movie?.overview || "");

	useEffect(() => {
		if (movie?.overview) {
			setUpdatedOverview(movie.overview);
		}
	}, [movie]);
	...
	const handleInputChange = (event) => {
		setUpdatedOverview(event.target.value);
	};
			...
			<div className = 'col-span-4'>
				...
				<form className="pt-8 flex flex-col gap-4" onSubmit={handleSubmit}>
					...
					<textarea
						...
						className="w-full p-2 text-black rounded shadow"
						rows="6"
					/>
